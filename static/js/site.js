window.addEventListener("DOMContentLoaded", () => {
  ...
  [...document.getElementsByTagName("nav")].forEach((x) =>
    registerNavigation(x, ["section", "fragment"])
  );
...
const traverseTree = (navElement) => {
  const ids = [];
  for (
    let element = navElement.parentNode;
    element !== null;
...
const registerNavigation = (navElement, childTagNames) => {
  ...
  const parentFragment =
    parentNavigation.length <= 0 ? "" : `/${parentNavigation.join("/")}`;
  const targetElements = [
    ...navElement.parentElement.childNodes.values(),
  ].filter((x) => childTagNames.includes(x.tagName?.toLowerCase()));
  const linkTargets = [...navElement.childNodes.values()].filter(
    (x) => x.tagName?.toLowerCase() === "a"
  );
